    ...
    <div class="py-8 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden" id="payslip-print">
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-8 py-8 text-white">
                <div class="text-center">
                    ...
                    <p class="text-blue-100 mt-2">{{ $payslip->period_label }}</p>
                </div>
            </div>
            ...
            <div class="px-8 py-8 space-y-6">
                ...
                <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-indigo-100 text-sm font-semibold uppercase">Net Pay</p>
                            <p class="text-3xl font-bold">₱ {{ number_format($payslip->net_pay, 2) }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-indigo-100 text-sm">Gross: ₱ {{ number_format($payslip->gross_pay, 2) }}</p>
                            <p class="text-indigo-100 text-sm">Deductions: ₱ {{ number_format($payslip->total_deductions, 2) }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6 flex gap-3">
            <button 
                ...
                class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-900 hover:bg-indigo-800 text-white font-medium rounded-lg transition-colors duration-200">
                Print Payslip
            </button>
            <a href="{{ route('employee.payslips.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 border border-gray-400 bg-gray-300 hover:bg-gray-400 text-black font-medium rounded-lg transition-colors duration-200">
                Back to Payslips
            </a>
        </div>
    </div>
